...
import { ChangePhoneAction, UserAction } from "../Redux/Actions/Actions";
...
const Checkout = () => {
  ...
  async function CheckoutProcess() {
    ...
    axios.post(UserRequests.PAYMENT, {
      amount: total
    })
      .then(response => {
        const options = {
          ...
          name: 'Payment',
          description: 'Thank you for paying online your products are on the way!',
          handler: function (response) {
            ...
            axios.post(UserRequests.PLACE_ORDER, obj)
              .then((response) => {
                if(response.data){
      ...
      <div className="checkout__page ">
        <div className="check__left">
              ...
              <details>
                ...
                <form className="MobileChangeForm">
                  <input
                    ...
                    type="number"
                  />
                  <input onClick={MobileChange} type="button" value="Update" />
                </form>
              </details>
            </div>
          </div>
          <div className="checkout__card">
            ...
            <div className="checkout__shipping" >
              <details>
                <summary onClick={() => CheckoutProcess()} style={{ cursor: 'pointer' }}>Upi/Debit Card</summary>
              </details>
              <form
                style={{
                  display: "flex",
              ...
              >
                ...
                <input type="checkbox" name="cod" id="checkbox" onClick={(e) => setCashOnDelivery(e?.target?.checked)} />
              </form>
              {
                paymentError
                  ?
                  <>
                    <span style={{ color: 'red', fontWeight: '500', fontSize: '1.1rem', marginTop: '10px', display: 'flex', justifyContent: 'center' }}>Select a Payment Method</span>
                  </>
                  :
                  <>
                  </>
          ...
          <div className="checkoutBtn resMob" style={{ cursor: 'pointer' }} onClick={() => PlaceOrder()}>
            <span>Checkout</span>
            <span>{total}</span>
          </div>
        </div>
                    ...
                    <>
                      {
                        userLocalCart
                          ...
                          userLocalCart.map((items) => {
                            let item = Products.find((item) => item?._id == items?.productId);
                            return <>
                              <div className="sub_payment">
        ...
        <div className="checkoutBtnMobile">
          <span>Checkout</span>
          <span>555</span>
        </div>
      </div>
